    ...
    public class ChatHub : Hub
    {
        ...
        public async Task SendMessageToSpecificUser(string message, string receiverGuid, string messageGuid)
        {
            try
            {
                ...
                var messageObj = await _messageApplicationService.CreateMessage(currentUserId, chatId, message, "", new Guid(messageGuid), receiverConnectionIds.Count == 0 ? MessageStatus.Saved : MessageStatus.Sent);
